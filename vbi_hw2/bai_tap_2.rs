...
fn main() {
    ...
    while additions.len() > 0 {
        ...
        //additions is moved here, therefore no additions in existence to loop through
            let addition_aux = values[*element_index]; //* is used to dereference the pointer, so that we can take the actual number
